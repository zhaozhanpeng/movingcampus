...
public class TimeUtil {
	...
	public TimeUtil(Activity ac,String key) {
        mPreferences = ac.getSharedPreferences(Constants.PREFER_FILE, Context.MODE_PRIVATE);
	...
	public long getUpdateTime() {
		if(mUpdateDateTime==0)
			return mUpdateDateTime=mPreferences.getLong(Constants.PREFER_FILE+ "_" + key, 0);
		return mUpdateDateTime;
	}

	/** 设置channel的刷新时间 **/
	private void setUpdateDateTime(long nowTime) {
		if (mPreferences.edit().putLong(Constants.PREFER_FILE+ "_" + key, nowTime).commit()) {
