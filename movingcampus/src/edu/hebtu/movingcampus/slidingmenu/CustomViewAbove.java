	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			try {
				...
				if (DEBUG)
					LogUtil.v(TAG, "onInterceptTouch moved to:(" + x + ", " + y
							+ "), diff:(" + xDiff + ", " + yDiff
							+ "), mLastMotionX:" + mLastMotionX);
				if (xDiff > mTouchSlop && xDiff > yDiff && thisSlideAllowed(dx)) {
					if (DEBUG)
						LogUtil.v(TAG, "Starting drag! from onInterceptTouch");
					startDrag();
					mLastMotionX = x;
					setScrollingCacheEnabled(true);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (!mIsBeingDragged) {
				...
				final float yDiff = Math.abs(y - mLastMotionY);
				if (DEBUG)
					LogUtil.v(TAG, "onTouch moved to:(" + x + ", " + y
							+ "), diff:(" + xDiff + ", " + yDiff
				...
				if ((xDiff > mTouchSlop || (mQuickReturn && xDiff > mTouchSlop / 4))
					...
					if (DEBUG)
						LogUtil.v(TAG, "Starting drag! from onTouch");
					startDrag();
					mLastMotionX = x;
					setScrollingCacheEnabled(true);
				} else {
					if (DEBUG)
						LogUtil.v(TAG, "onTouch returning false");
					return false;
				}
			}
			if (mIsBeingDragged) {
