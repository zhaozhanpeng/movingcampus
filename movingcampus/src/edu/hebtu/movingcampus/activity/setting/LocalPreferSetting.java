...
public class LocalPreferSetting extends BaseActivity {
	...
	public void onCreate(Bundle b) {
		...
		// 5-100
		SharedPreferences pre = getSharedPreferences(Constants.PREFER_FILE, 0);
		seekBarlib = (SeekBar) findViewById(R.id.SeekBarlib);
		seekBarlib
				.setProgress((int) ((100.0 * pre.getInt(Constants.LIB_DAYS, 10)- libmin) / (1.0 * (libmax - libmin))));
		libtip = (TextView) findViewById(R.id.lib_num);

		libtip.setText(pre.getInt(Constants.LIB_DAYS, 10) + "天");

		// 3-30
		seekBarcard = (SeekBar) findViewById(R.id.seekBarcard);
		seekBarcard
				.setProgress((int) (100.0 * (pre.getInt(Constants.BALANCE_LOWEAST, 10)- cardmin) / (1.0 * (cardmax - cardmin))));
		cardtip = (TextView) findViewById(R.id.card_num);
		cardtip.setText(pre.getInt(Constants.BALANCE_LOWEAST, 10) + "元");
	...
	public void bindButton() {
		...
		seekBarlib.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
			...
			public void onProgressChanged(SeekBar arg0, int progress,
				...
				libtip.setText((int) (progress * 1.0 / 100.0
						* (libmax - libmin) + libmin)
						+ "天");
			}
			...
			public void onStopTrackingTouch(SeekBar bar) {
				SharedPreferences.Editor editor = LocalPreferSetting.this
						.getSharedPreferences(Constants.PREFER_FILE, 0).edit();
				int days=(int) (bar.getProgress() * 1.0 / 100.0
						* (libmax - libmin) + libmin);
				editor.putInt(Constants.LIB_DAYS, days);
				editor.commit();
			}
		});
		...
		seekBarcard.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
			...
			public void onProgressChanged(SeekBar arg0, int progress,
				...
				cardtip.setText((int) (1.0 * progress / 100.0
						* (cardmax - cardmin) + cardmin)
						+ "元");
			}
			...
			public void onStopTrackingTouch(SeekBar bar) {
				SharedPreferences.Editor editor = LocalPreferSetting.this
						.getSharedPreferences(Constants.PREFER_FILE, 0).edit();
				int loweast = (int) (1.0 * bar.getProgress() / 100.0
						* (cardmax - cardmin) + cardmin);
				editor.putInt(Constants.BALANCE_LOWEAST, loweast);
				editor.commit();
			}
		});
	}
